...
//    public static final String FULL_PATH_TRAIN = "E:\\FinalProject\\datasets\\trec06c\\full\\index_train";
//    public static final String FULL_PATH_PREDICT = "E:\\FinalProject\\datasets\\trec06c\\full\\index_pre";
//    public static final String DATA_PATH = "E:\\FinalProject\\datasets\\trec06c";
//    public static final String MODEL_PATH = "E:\\FinalProject\\models\\FeatureNumModels\\TFNB";
...
//
//        /************读取训练集****************/
//        ArrayList<String> spamMailListTrain  = new ArrayList<>();
//        ArrayList<String> hamMailListTrain   = new ArrayList<>();
//        /*****************read the type and path of all emails*******************************/
//        Map<String, String> typeAndPathMapTrain = ProcessFile.getTypeAndPath(FULL_PATH_TRAIN);
//        /**********************read spam as will as ham and put them into spamMailList and hanMailList*****************/
//        Integer spamNumTrain = 0;
//        Integer hamNumTrain  = 0;
//        int k = 1;
//        for (Map.Entry<String,String> entry: typeAndPathMapTrain.entrySet()){
//            String key = entry.getKey();
//            String value = entry.getValue();
//            String path = null;
//            //put spam into spamMailList
//            if (value.equals("spam") && (spamNumTrain++)<SPAM_NUM_TRAIN){
//                System.out.println("正在读取第"+(k++)+"封邮件："+key+"\t该邮件为："+value);
//                path = key.replace("..",DATA_PATH);
//                //remove this sentence when run on linux
//                path = path.replace("/", "\\");
//                String mail = null;
//                mail = ProcessFile.readFile(path);
//                spamMailListTrain.add(mail);
//            }
//            if(value.equals("ham") && (hamNumTrain++)<HAM_NUM_TRAIN) {
//                System.out.println("正在读取第"+(k++)+"封邮件："+key+"\t该邮件为："+value);
//                path = key.replace("..",DATA_PATH);
//                //remove this sentence when run on linux
//                path = path.replace("/", "\\");
//                String mail = null;
//                mail = ProcessFile.readFile(path);
//                hamMailListTrain.add(mail);
//            }
//            if (hamNumTrain>HAM_NUM_PREDICT&&spamNumTrain>SPAM_NUM_PREDICT){
//                break;
//            }
//        }
//        System.out.println("**********************训练集分词********************");
//        ArrayList<ArrayList<String>> spamWordsListTrain = HanlpProcess.cutWords(spamMailListTrain);
//        ArrayList<ArrayList<String>> hamWordsListTrain = HanlpProcess.cutWords(hamMailListTrain);
//
//        /*****************remove stop words——新版*******************************/
//        System.out.println("**********************训练集去除停用词********************");
//        ArrayList<ArrayList<String>> keySpamWordsTrain = RemoveStopWords.getKeyWordsList(spamWordsListTrain);
//        ArrayList<ArrayList<String>> keyHamWordsTrain = RemoveStopWords.getKeyWordsList(hamWordsListTrain);
//
//        //读取测试集数据
//        //获取垃圾邮件和正常邮件，并放入列表中
//        /*****************read the type and path of all emails*******************************/
//        Map<String, String> typeAndPathMapPre = ProcessFile.getTypeAndPath(FULL_PATH_PREDICT);
//        /**********************read spam as will as ham and put them into spamMailList and hanMailList*****************/
//        Integer spamNumPre = 0;
//        Integer hamNumPre  = 0;
//        int i_pre = 0;
//        int j_pre = 0;
//        int k_pre = 1;
//        ArrayList<String> spamMailListPre  = new ArrayList<>();
//        ArrayList<String> hamMailListPre   = new ArrayList<>();
...
//        for (Map.Entry<String,String> entry: typeAndPathMapPre.entrySet()){
//            //key是文件相对路径
//            String key = entry.getKey();
//            //value是文件的类型
//            String value = entry.getValue();
//            String path = null;
//            //put spam into spamMailList
//            if (value.equals("spam") && (spamNumPre++)<SPAM_NUM_PREDICT){
...
//                String mail = null;
//                mail = ProcessFile.readFile(path);
//                spamMailListPre.add(mail);
//            }
//            if(value.equals("ham") && (hamNumPre++)<HAM_NUM_PREDICT) {
//                System.out.println("正在读取第"+(k_pre++)+"封邮件："+key+"\t该邮件为："+value);
...
//                String mail = null;
//                mail = ProcessFile.readFile(path);
//                hamMailListPre.add(mail);
//            }
//            if (hamNumPre>HAM_NUM_PREDICT&&spamNumPre>SPAM_NUM_PREDICT){
//                break;
//            }
//        }
...
//        ArrayList<ArrayList<String>> spamWordsPre = HanlpProcess.cutWords(spamMailListPre);
//        ArrayList<ArrayList<String>> hamWordsPre = HanlpProcess.cutWords(hamMailListPre);
//
//
//        /*****************循环训练，预测，找到合适的特征数量*****************/
//        for (;featureNum<=FEATURE_NUM_MAX; featureNum += FEATURE_NUM_INCREMENT){
//            //更新特征数量
//            System.out.println("特征数量："+featureNum);
//            // 对分词列表进行词频统计获取TOPN数据
//            System.out.println("**********************从训练集中获取TOPN********************");
//            List<String> spamTopN = getTopN(keySpamWordsTrain, featureNum/2);
//            List<String> hamTopN = getTopN(keyHamWordsTrain, featureNum/2);
//
//            // 合并TOPN数据得到特征数据TOP2N
...
//            }
//
//            System.out.println("**********************对训练集列表进行特征匹配生成训练数据********************");
//            ArrayList<LabeledPoint> spamLabeledPointListTrain = getTrainData(keySpamWordsTrain, allKeyWord, 1.0, featureNum);
//            ArrayList<LabeledPoint> hamLabeledPointListTrain = getTrainData(keyHamWordsTrain, allKeyWord, 0.0, featureNum);
//            spamLabeledPointListTrain.addAll(hamLabeledPointListTrain);
//            // 将训练数据转化为rdd
//            RDD<LabeledPoint> trainRDD = javaSparkContext.parallelize(spamLabeledPointListTrain).rdd();
//            // 数据训练生成模型
...
//
//
//            System.out.println("*********************测试集生成测试数据*********************");
//            ArrayList<LabeledPoint> spamLabeledPointPre = getTrainData(spamWordsPre, allKeyWord, 1.0, featureNum);
//            ArrayList<LabeledPoint> hamLabeledPointPre = getTrainData(hamWordsPre, allKeyWord, 1.0, featureNum);
//            spamLabeledPointPre.addAll(hamLabeledPointPre);
//            //将LabeledPoint装华为RDD
////            System.out.println("********************正在将LabeledPoint转化为RDD**********************");
////            RDD<LabeledPoint> labeledPointRDD = javaSparkContext.parallelize(spamLabeledPointPre).rdd();
//            //加载持久化的训练模型
...
//            int TN = 0;
//            int FN = 0;
//            for (LabeledPoint labeledPoint:spamLabeledPointPre){
...
//            double accuracy = Double.valueOf(decimalFormat.format((double)(spamLabeledPointPre.size() - wrong) / spamLabeledPointPre.size()))*100;
...
//
//            System.out.println("======================");
//            System.out.println("预测完毕！一共预测" + spamLabeledPointPre.size() + "封邮件！\r\n" +
//                            "预测准确：" + (spamLabeledPointPre.size() - wrong) + "封邮件！\r\n" +
//                            "预测错误：" + wrong + "封邮件！\r\n" +
//                            "预测准确率为：" + accuracy + "！\r\n"
...
//    }
    ...
    public void trainAndPre() {
        ...
        for (;featureNum<=FEATURE_NUM_MAX; featureNum += FEATURE_NUM_INCREMENT){
            ...
            List<String> spamTopN = getTopN(keySpamWords, featureNum/2);
            List<String> hamTopN = getTopN(keyHamWords, featureNum/2);
...
//            }
            ...
            ArrayList<LabeledPoint> spamLabeledPointList = getTrainData(keySpamWords, allKeyWord, 1.0, featureNum);
            ArrayList<LabeledPoint> hamLabeledPointList = getTrainData(keyHamWords, allKeyWord, 0.0, featureNum);
            ...
            ArrayList<LabeledPoint> spamLabeledPointList_pre = getTrainData(spamWords, allKeyWord, 1.0, featureNum);
            ArrayList<LabeledPoint> hamLabeledPointList_pre = getTrainData(hamWords, allKeyWord, 0.0, featureNum);
